...
var vlog = "v1.1.2-> New Update for fix normal `lo`\nv1.1.1 -> fix Some Bug And Remove `-1` you can use just `lo`\nv1.0.10 -> `-1a` chenged to `-a`\nv1.1.0 -> fix bug and some erorrs and remove `-n` "
var verion = "v1.1.2"
...
func main() {
	...
	if len(os.Args) == 2 || len(os.Args) == 3 || len(os.Args) == 4 {
		...
		for _, i := range list {
			...
			if string(filename[0]) == "." {
				continue
			} else if i.IsDir() { //is Dir? yes Print Dir No Dont Print and (.) is hiddin and dont Print
				fmt.Printf("📂 %v\n", i.Name())
			} else {
				continue
			}
